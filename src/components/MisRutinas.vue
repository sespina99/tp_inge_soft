  ...
  <div>
    <v-row>
      ...
      <v-col cols="2" class="pa-2">
        <h1 class="text-center">Mis Rutinas</h1>
      </v-col>
      <v-col cols="1">
        <v-spacer></v-spacer>
    ...
    <v-row>
      ...
      <v-col cols="8" class="pa-2">
        <carrousel v-bind:routines="filteredRoutines" v-on:borradoDeRutina="actualizarRutinas()"></carrousel>
      </v-col>
      <v-spacer></v-spacer>
    </v-row>
  </div>
...
export default {
  ...
  methods:{
    async actualizarRutinas(){
      this.routines = await this.store.getMyRoutines('dateCreated');
    },
    getOrder() {
      switch (this.order) {
    ...
    async updateRoutines() {
      ...
      if (aux === 'difficulty') {
        ...
        this.routines = await this.store.getMyRoutines(aux);
    },
  },
